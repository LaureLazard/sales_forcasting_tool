...
from lib_interface import datetime, date, plt, np, pd, Figure
...
class ScatterPlot(wx.Panel):
    ...
    def __init__(self, parent, columns, df_list_ctrl):
        ...
        self.line_btn = wx.Button(self, -1, "Regression line")
        self.Bind(wx.EVT_COMBOBOX, self.on_combo_box_select)
        self.line_btn.Bind(wx.EVT_BUTTON,self.OnClicked) 
        row_sizer = wx.BoxSizer(wx.HORIZONTAL)
        row_sizer.Add(self.combo_box1, 0, wx.ALL | wx.ALIGN_CENTER, 5)
        row_sizer.Add(self.combo_box2, 0, wx.ALL | wx.ALIGN_CENTER, 5)
        row_sizer.Add(self.line_btn, 0, wx.ALL | wx.ALIGN_CENTER, 5)
        row_sizer.Add(chart_toolbar, 0, wx.ALL, 5)
    ...
    def OnClicked(self, event): 
        ...
        column_index2 != wx.NOT_FOUND and column_index2 != 0:
            column_name1 = self.combo_box1.GetValue()
            column_name2 = self.combo_box2.GetValue()
            self.addline(column_name1, column_name2)
    ...
    def addline(self, X, y ):
        ...
        print(df[X])
        print(df[y])
        Xr = df[X].values.reshape(-1,1)
        yr = df[y].values.reshape(-1,1)
        ...
        self.axes.clear()
        self.axes.scatter(df[X], df[y],  color='gray')
        self.axes.plot(X_test, y_pred, color='red', linewidth=4)
        self.canvas.draw()
